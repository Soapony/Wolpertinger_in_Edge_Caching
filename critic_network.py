...
class critic_network:
    def __init__(
        ...
        if self.DEBUG:
            ...
            print("layer1 units = ",self.layer1_units)
            print("layer2 units = ",self.layer2_units)
            print("model summary:")
    ...
    def create_critic_network(self):
        ...
        L1_state = Dense(self.l_units128, name="critic_state_L1", activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(input_state)
        L2_state = Dense(self.l_units64, name="critic_state_L2", activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(L1_state)
        L3_state = Dense(self.l_units32, name="critic_state_L3", activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(L2_state)
        
        L1_action = Dense(self.l_units32, name="critic_action_L1")(input_action)

        concat = Concatenate()([L3_state, L1_action])
        ...
        L3_concat = Dense(self.l_units32, name="critic_concat_L3", activation='relu', kernel_initializer=tf.keras.initializers.HeNormal())(L2_concat)

        output = Dense(1, name="critic_out", activation='linear')(L3_concat)
